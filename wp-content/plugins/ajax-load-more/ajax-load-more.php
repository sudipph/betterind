...
function alm_create_table(){
	...
	$defaultRepeater = '<li  <?php if (!has_post_thumbnail()) { ?> class="no-img"<?php } ?>>'. PHP_EOL .'   <?php if ( has_post_thumbnail() ) { the_post_thumbnail(\'alm-thumbnail\'); }?>'. PHP_EOL .'   <h3><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_title(); ?></a></h3>'. PHP_EOL .'   <p class="entry-meta"><?php the_time("F d, Y"); ?></p>'. PHP_EOL .'   <?php the_excerpt(); ?>'. PHP_EOL .'alm_create_table</li>';
      ...
      */
   	...
   	*/
   	...
   	public function alm_query_posts() {
		 ...
		 //custom add
		//  $m['key'] = 'gallery_data';
		//  $m['value'] = sprintf(':"%s";', 'top-stories');
		//  $m['compare'] = 'like';
		//  $a['relation'] = 'or';
		//  $a[]=$m;
		//  $args['meta_query'] = $a;
		 //custom add
		 //taxonomy add code by sudip
		 $m['taxonomy'] = 'placement';
		 $m['terms'] = 'top-stories';
		 $m['field'] = 'slug';
		 $a['operator'] = 'IN';
		 $a[]=$m;
	            ...
	            // ALM Loop
	   			...
	   			while ($alm_query->have_posts()): $alm_query->the_post();
	   	         ...
	   	         $alm_item = ($alm_page_count * $posts_per_page) - $posts_per_page + $alm_loop_count; // Get current item
                     ...
                     //echo alm_get_current_repeater( $repeater, $type );
