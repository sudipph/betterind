<?php
/*
Plugin Name: Success Story Submission
...
function custom_table_example_install()
{
    ...
    $table_name = $wpdb->prefix . 'success_story'; // do not forget about tables prefix
    ...
    $sql = "CREATE TABLE " . $table_name . " (
        ...
        solutions_library VARCHAR(200) NOT NULL,
        story text NULL,
        PRIMARY KEY  (id)
    );";
...
class Custom_Table_Example_List_Table extends WP_List_Table
{
    ...
    function __construct()
    {
        ...
        parent::__construct(array(
            'singular' => 'success-story-submission',
            'plural' => 'success-story-submission',
        ));
    }
    ...
    /**
        ...
        * @return HTML
        */
    function column_story($item)
    {
        if(isset($item['post_id']) && $item['post_id']!=0){
            return '<em>' . get_the_title( $item['post_id']). '</em>';
        }else{
            return '<em>' .  $item['solutions_library']. '</em>';
        }
    ...
    function column_name($item)
    {
        ...
        $actions = array(
            'edit' => sprintf('<a href="?page=story_form&id=%s">%s</a>', $item['id'], __('Edit', 'custom_table_example')),
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'story' => __('Story From (Solution)', 'custom_table_example'),
        );
        return $columns;
    }
...
{
    add_menu_page(__('Success Stories', 'custom_table_example'), __('Success Stories', 'custom_table_example'), 'activate_plugins', 'story', 'custom_table_example_persons_page_handler','',6);
    add_submenu_page('Story Submission', __('Story Submission', 'custom_table_example'), __('Story Submission', 'custom_table_example'), 'activate_plugins', 'story', 'custom_table_example_persons_page_handler');
    //add_submenu_page('Story Submission', __('Story Submission', 'custom_table_example'), __('Story Submission', 'custom_table_example'), 'activate_plugins', 'front-sections-future', 'custom_table_example_persons_page_handler');
    //add_submenu_page('Story Submission', __('Story Submission', 'custom_table_example'), __('Story Submission', 'custom_table_example'), 'activate_plugins', 'front-sections-future', 'custom_table_example_persons_page_handler');
   
    //front-sections-future
    // add new will be described in next part
    add_submenu_page('story', __('Story Submission', 'custom_table_example'), __('Story Submission', 'custom_table_example'), 'activate_plugins', 'story', 'custom_table_example_persons_form_page_handler');
    add_submenu_page('story', __('Add Submission ', 'custom_table_example'), __('Add Submission', 'custom_table_example'), 'activate_plugins', 'story_form', 'custom_table_example_persons_form_page_handler');
...
<div class="wrap">
    ...
    <h2><?php _e('Success Story Submission', 'custom_table_example')?> <a class="add-new-h2"
                                    href="<?php echo get_admin_url(get_current_blog_id(), 'admin.php?page=story_form');?>"><?php _e('Add new', 'custom_table_example')?></a>
    </h2>
    <?php echo $message; ?>
...
<table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
    ...
    <?php if(isset($item['post_id']) && $item['post_id']!=0){?>
    <tr class="form-field">
        <th valign="top" scope="row">
            <label for="post-id"><?php _e('Solution', 'custom_table_example')?></label>
        </th>
        <td>
    ...
    <?php }?>
    <?php if(isset($item['solutions_library'])){?>
    <tr class="form-field">
        <th valign="top" scope="row">
            <label for="post-id"><?php _e('Solution Library', 'custom_table_example')?></label>
        </th>
        <td>
        <?php echo $item['solutions_library'];?>    
        </td>
    </tr>
    <?php }?>
    <tr class="form-field">
        <th valign="top" scope="row">
            <label for="story"><?php _e('Story', 'custom_table_example')?></label>
        </th>
        <td>
            <textarea name="story" class="code" id="story" placeholder="<?php _e('Success Story', 'custom_table_example')?>"><?php echo esc_attr($item['story'])?></textarea>
...
function custom_table_example_validate_person($item)
{
    ...
    if (empty($item['story'])) $messages[] = __('Story can\'t be blank', 'custom_table_example');
