...
if ( ! class_exists( 'evolve_Kirki_child' ) ) {
	class evolve_Kirki_child {
		static function setSection( $param1, $param2 ) {
			if ( true || is_user_logged_in() ) {
				...
				if ( isset( $param2['fields'] ) && is_array( $param2['fields'] ) && count( $param2['fields'] ) ) {
					...
					evolve_Kirki_child::call_kirki_from_old_field( $param2['fields'], $param2['id'] );
				} else {
					$evolve_panel = $param2['id'];
					if ( is_user_logged_in() && is_customize_preview() ) {
...
if ( ! is_customize_preview() ) {
	...
	if ( ! function_exists( 'evolve_enqueue_google_fonts' ) ) {
		function evolve_enqueue_google_fonts() {
			...
			wp_register_style( 'evolve-google-fonts', $protocol . evolve_Kirki_child::google_webfont_url( $evolve_list_google_fonts ), '' );
