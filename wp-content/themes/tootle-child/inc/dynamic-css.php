...
if ( ! function_exists( 'evolve_dynamic_css' ) ) {
	function evolve_dynamic_css( $css_data ) {
		...
		if ( ! ( '' == evolve_theme_mod( 'evl_menu_back_color', '#273039' ) ) ) {
			if(is_page('home')){
					$css_data .= ' @media (min-width: 768px) { header.dinamic_background_image .header-overlay{ height:694px;} }';
				}
		...
		if ( get_header_image() ) {
			$css_data .= ' .custom-header {	background-image: url(); background-position: ' . $background_position . '; background-repeat: ' . $background_repeat . '; position: relative; background-size: ' . $header_image . '; width: 100%; height: 100%; }';
		}
		...
		if ( ( is_front_page() && is_page() ) || is_home() ) {
			if ( $width_px && ( $frontpage_width_layout == "fixed" ) ) {
				$css_data .= ' .container, #wrapper { width: 100%; max-width: unset; }';
			} else {
