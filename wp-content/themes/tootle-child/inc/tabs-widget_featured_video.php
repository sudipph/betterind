<?php
define('WIDGET_TITLE_CATEGORY', 'Featured Video Widget');
define('WIDGET_TITLE_CATEGORY_SLUG', 'recent-posts-plus-category');

class RecentPostsPlus_category extends WP_Widget {
    ...
    function __construct() {
        ...
        parent::__construct(WIDGET_TITLE_CATEGORY_SLUG, __(WIDGET_TITLE_CATEGORY), $widget_ops);
    }
 ...
 $title = apply_filters( 'widget_title', empty($instance['title']) ? '' : $instance['title'], $instance, $this->id_base);        
         if(trim($title)!=''){
                    echo '<div class="featured_video_box_for_desktop_view">';
                    echo '</div>';
                    echo '<div class="featured_video_box">';
...
$video_slider_thumbnail = true;
        ...
        //var_dump(array_merge($default_query_args, $query_args));
        ...
        if( $the_query->have_posts() ) {
            ...
            while ( $the_query->have_posts() ) { $the_query->the_post();
                ...
                if($video_slider_thumbnail=='true'){
                    ...
                    $videotitle = $widget_ouput_template_params['{TITLE}'];
                   $videotitle = custom_echo($videotitle,100);
                    $output .= '<div class="col-12"><div class="video_content_with_date"><span class="video_title tab_widget_featured_video">'.$videotitle.'</span><span class="video_date">'.$widget_ouput_template_params['{DATE}'].'</span></div>';
                    $gallery =  get_post_meta( $ID , 'gallery_data' , true ); 
                    if(isset($gallery['youtube_link'])){
                        $link_embed = $this->youtube_link_to_embeded_code($gallery['youtube_link']);
                        //var_dump($link_embed);
                    $output .= '<div class="col-12 video_slide_youtube">'.$link_embed.'</div>';
                    }
                    $output .= '<span class="social_icons">
                    <div class="row">
                        '.social_icon_with_share().'
                    </div>
                    <!--<div id="u87" class="ax_default icon">
                    <img id="u87_img" class="img" src="'.get_stylesheet_directory_uri() .'/images/u87.png">
                  </div><div id="u88" class="ax_default icon mouseOver">
                  <img id="u88_img" class="img mouseOver" src="'.get_stylesheet_directory_uri() .'/images/u88_mouseOver.png">
                </div><div id="u89" class="ax_default icon">
                <img id="u89_img" class="img " src="'.get_stylesheet_directory_uri() .'/images/u89.png">
              </div>-->
              </span>';
                    $output .= '</div>';
                    $output .= '</div>';
       ...
       console.log(jQuery(this).val());
        ...
        <!-- <p>
            <?php if( current_theme_supports('post-thumbnails') ): ?>
                ...
                <label for="<?php echo $this->get_field_id('video_slider_thumbnail'); ?>"><?php _e('Video Slider'); ?></label><br>
            <?php endif; ?>
...
add_action( 'admin_enqueue_scripts', array('RecentPostsPlus_category', 'rpp_form_script') );


// register RecentPostsPlus widget
add_action( 'widgets_init', 'create_widget_function' );

function create_widget_function(){
    
    register_widget("RecentPostsPlus_category");
}
