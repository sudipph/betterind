...
//echo $type ;
        ...
        <form action="<?php echo home_url(); ?>" method="get" class="header-search search-form<?php echo $class; ?>">
            <label>
                <input type="text" aria-label="<?php _e( "Search", "evolve" ); ?>" name="s" class="form-control search_text_from_home"
                       placeholder="<?php esc_html_e( 'Type your search', 'evolve' ); ?>"/>

				<?php //echo evolve_get_svg( 'search' ); ?>
				<div id="u8" class="ax_default icon">
	              <!-- <img id="u8_img" class="img " src="<?php echo get_stylesheet_directory_uri();?>/images/u8.png"> -->
	            </div>
				<div id="u9" class="ax_default" tabindex="0" style="cursor: pointer;">
                </div>
            </label>
        </form>
...
if ( ! function_exists( 'evolve_sticky_header_open_children' ) ) {
	function evolve_sticky_header_open_children() {
		global $post;
		//var_dump($post->post_type);
		//die();
		//var_dump(evolve_theme_mod( 'evl_sticky_header', true ));
                    ...
                    <div class="col-auto">
                        <div class="row align-items-center">
							...
							if ( evolve_theme_mod( 'evl_blog_title', '0' ) == "0" ) { ?>
                                <div class="<?php echo( '' != ( evolve_theme_mod( 'evl_header_logo', '' ) && evolve_theme_mod( 'evl_pos_logo', 'left' ) != "disable" ) ? 'col-auto pr-0' : 'col-auto' ) ?> logo_for_desktop_view">
        ...
        <div class="header-height to_build_dynamic_header_banner <?php echo isset($post->post_type)?$post->post_type.' ':'';echo get_post_field( 'post_name', get_post() );?> <?php echo $solarticle?>">

	<?php  }
}

add_action( 'evolve_header_area_children', 'evolve_sticky_header_open_children', 20 );
...
add_action( 'evolve_header_area_children', 'evolve_sticky_header_close', 50 );
...
add_action( 'evolve_header_area_children', 'evolve_header_block_above', 30 );
...
add_action( 'evolve_header_area_children', 'evolve_header_block_below', 60 );
...
if ( ! function_exists( 'evolve_custom_footer' ) ) {
	function evolve_custom_footer() {
		...
		$evolve_home_url = get_site_url().'/';
		echo '<div class="row"><div class="col custom-footer">' . evolve_theme_mod( 'evl_footer_content', '<div id="copyright">The Better India Powered by <a href="http://mytootle.com/" target="_blank">TOOTLE</a></div>' ) . '</div></div>';
	}
}
...
if ( ! function_exists( 'evolve_post_meta' ) ) {
	function evolve_post_meta( $type = '',$class_param='' ) {
		if ( $type == "header" ) {
			...
			if ( ! is_page() && ( evolve_theme_mod( 'evl_header_meta', 'single_archive' ) == "single_archive" || ( evolve_theme_mod( 'evl_header_meta', 'single_archive' ) == "single" && is_single() ) ) ) {
				...
				echo '<div class="col author vcard template_tags">';
				...
				//echo '<div class="share_beside_author">'.share().'</div>	';
			...
			?>
			<div class="row post-meta align-items-center <?php echo $class_param;?>">
				<div class="author vcard template_tags_1">
					...
					<div class="share_beside_author <?php echo $class_param;?>"><?php share();?></div>	
				</div>
