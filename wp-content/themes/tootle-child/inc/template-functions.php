...
if ( ! function_exists( 'evolve_wrapper_container_open' ) ) {
	function evolve_wrapper_container_open() {
		echo '<div class="top_header_area_for_desktop"><div class="middle_image"><div class="middle_image_inner"><a id="sticky-title" href="'. home_url().'"><img src="'.get_stylesheet_directory_uri().'/images/u137.png"></a></div></div></div>';
		echo '<div id="wrapper"' . evolve_wrapper_class() . '>';
	}
}

add_action( 'evolve_header_area_children', 'evolve_wrapper_container_open', 10 );
...
if ( ! function_exists( 'evolve_content_container_open' ) ) {
	function evolve_content_container_open() {
		global $post,$get_meta_val,$ct,$post_type;
...
$placement =  implode(' ',$terms_placement);
$class_helper .= ' '.$placement .' ';
$class_helper .= isset($get_meta_val['card_type'])?$get_meta_val['card_type']:'';
//echo $get_meta_val['card_type'];

$class_helper .= ' content-post';
$class_helper .= ' '.$post->post_name;
if(isset($_GET['placement']) && $_GET['placement']!=''){
	$class_helper .= ' '.$_GET['placement'];
}
$class_helper .= ' '.$ct;
$class_helper .= ($post_type!=''?' post_t_'.$post_type:'');

//echo $class_helper;
		//echo $post->post_name;
		//echo get_the_title($post->ID);
		if ( ( ( is_front_page() ) && evolve_theme_mod( 'evl_front_elements_content_display', 'above' ) != 'above' ) || ! is_front_page() ) {
			echo '<div class="content '.$class_helper.'"><div class="container '.$post->post_name.'"><div class="row '.$class_helper.'">';
			if($post_type=='quickbyte'){
				//echo '<div class="row '.$class_helper.' for_desktop_view"></div>';
			}
...
add_action( 'evolve_header_area_children', 'evolve_content_container_open', 70 );
...
add_action( 'evolve_header_area_children', 'evolve_header_type', 40 );
...
if ( ! function_exists( 'evolve_posts_loop_open' ) ) {
	function evolve_posts_loop_open() {
		if ( ( is_home() || is_archive() || is_search() ) && evolve_theme_mod( 'evl_post_layout', 'two' ) != "one" ) {
			global $post;
			echo '<div class="posts card-columns '.$post->post_name.'">';
		}
	}
}
...
//add_action('init','remove_action_init');
function remove_action_init(){
	remove_action( 'evolve_after_post_content', 'evolve_comments_template' ,10);
}
			...
			switch ( $elementval ) {
				...
				case 'blog_post':
					if ( $elementval ) {
						...
						<?php }
						//echo 'evolve_blog_page_content';
						evolve_blog_page_content_new();
							//echo 'evolve_blog_page_content';
						if ( evolve_theme_mod( 'evl_front_elements_content_display', 'above' ) == 'above' ) { ?>
                            </div><!-- #primary -->
